...
module.exports = exports = function postPlugin(schema /*, schemaOptns*/ ) {
   ...
   * 	...called before save(does not touch mongoose hooks)
   * 	done();
   * };
   *
   * UserSchema.methods.afterPost = function (done) {
   * 	...called after saved(does not touch mongoose hooks)
   * 	done();
   * };
   * 
  ...
  schema.methods.post = function post(done) {
    ...
    async.waterfall([
      ...
      function beforePost(next) {
        //obtain before hooks
        const before = (this.beforePost || this.beforeSave);
      ...
      function afterPost(instance, next) {
        //obtain after hooks
        const after = (instance.afterPost || instance.afterSave);
