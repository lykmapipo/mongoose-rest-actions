...
const updatesFor = (id, updates) => {
  ...
  if (mongooseCommon.isInstance(updates) && updates._id === id) {
    return updates;
  }
  //  compute updates
  const changes = common.mergeObjects(mongooseCommon.copyInstance(updates), { _id: id });
  return changes;
};
...
function patchPlugin(Schema) {
  ...
  schema.statics.patch = function patch(id, updates, done) {
    ...
    async.waterfall(
      [
        function findExisting(next) {
          if (mongooseCommon.isInstance(model)) {
            next(null, model);
          } else {
...
function postPlugin(Schema) {
  ...
  schema.statics.post = function post(body, done) {
    // instantiate model
    const instance = mongooseCommon.isInstance(body) ? body : new this(mongooseCommon.copyInstance(body));
